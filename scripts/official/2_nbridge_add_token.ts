	...
	for (const n of networkToChange) {
		...
		switch (network.config.chainId!) {
			case Number(originChainId):
				await setupNBridge(nb, originToken, Number(originChainId), crosserKey, crosser, feeTo, tokenSymbol)
...
async function setupNBridge(nb: NBridge, originToken: string, originChainId: number, crosserKey: string, crosser: string, feeTo: string,  tokenSymbol: string) {
